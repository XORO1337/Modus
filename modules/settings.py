...
class Settings(Gtk.Window):
    ...
    def _build_general_page(self, config):
        ...
        terminal_entry.set_text(config.get("terminal_command", "kitty -e"))
        ...
        adj = Gtk.Adjustment(
            value=config.get("window_switcher_items_per_row", 10),
        ...
        special_ws_switch.set_active(config.get("hide_special_workspace", True))
        self._widgets["hide_special_workspace"] = special_ws_switch
        box.pack_start(
            self._row(
                "Hide special workspace",
                special_ws_switch,
                "Hide the Hyprland special workspace from workspace bar",
            ),
            False, False, 0,
        )
    ...
    def _build_appearance_page(self, config):
        ...
        box.pack_start(self._section_label("Wallpapers"), False, False, 0)

        wallpaper_entry = Gtk.Entry()
        wallpaper_entry.set_name("settings-entry")
        wallpaper_entry.set_text(
            config.get("wallpapers_dir", "~/Pictures/Wallpapers/")
        )
    ...
    def _build_dock_page(self, config):
        ...
        current_pos = config.get("dock_position", "Bottom")
        ...
        enabled_switch.set_active(config.get("dock_enabled", True))
        ...
        autohide_switch.set_active(config.get("dock_auto_hide", True))
        ...
        occluded_switch.set_active(config.get("dock_always_occluded", False))
        ...
        adj = Gtk.Adjustment(
            value=config.get("dock_icon_size", 52),
        ...
        hide_special_switch.set_active(
            config.get("dock_hide_special_workspace_apps", True)
        )
    ...
    def _build_notifications_page(self, config):
        ...
        timeout_entry.set_text(config.get("notification_timeout", "5s"))
        ...
        box.pack_start(self._section_label("Ignored Apps"), False, False, 0)

        ignored_entry = Gtk.Entry()
        ignored_entry.set_name("settings-entry")
        ignored_val = config.get("notification_ignored_apps_history", ["Hyprshot"])
        ...
        limited_val = config.get("notification_limited_apps_history", ["Spotify"])
    ...
    def _on_apply(self, button):
        ...
        config["dock_always_occluded"] = self._widgets[
            "dock_always_occluded"
        ].get_active()
        config["dock_icon_size"] = int(
            self._widgets["dock_icon_size"].get_value()
        )
        ...
        # Notifications
        config["notification_timeout"] = self._widgets[
